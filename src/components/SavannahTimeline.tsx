...
import SafariCar from '@/components/SafariCarJourney';
...
const EXPEDITIONS = [
  {
    ...
    subtitle: 'Greatest Show on Earth',
    description: 'Millions of wildebeest cross the Mara River in a thundering spectacle. Balloon safaris at sunrise, golden hour with lion prides.',
    tags: ['Wildlife', 'Balloon', 'Migration'],
    stop: 1,
  },
  {
    id: 'amboseli',
    title: 'Amboseli Elephants',
    subtitle: 'Shadow of Kilimanjaro',
    description: "Africa's largest elephant herds roam freely beneath the snowcapped peak — a photographer's dream.",
    tags: ['Elephants', 'Kilimanjaro', 'Views'],
    stop: 2,
  },
  {
    id: 'tsavo',
    title: 'Tsavo Red Earth',
    subtitle: 'The Untamed Wilderness',
    description: "One of Africa's largest parks. Red dust-coated elephants, volcanic landscapes, raw untouched wilderness.",
    tags: ['Rugged', 'Red Elephants', 'Wilderness'],
    stop: 3,
  },
];
...
export default function SavannahTimeline() {
  ...
  useLayoutEffect(() => {
    ...
    const init = () => {
      ...
      if (mainAnimation) {
        gsap.utils.toArray<HTMLElement>('.sv-panel').forEach((panel) => {
    ...
    return () => {
      ...
      ScrollTrigger.getById('sv-main')?.kill();
      ScrollTrigger.getById('sv-video')?.kill();
      ScrollTrigger.getById('sv-path')?.kill();
  ...
  return (
    <section
      ref={sectionRef}
      id="savannah-section"
      className="relative bg-[#0a0f07] overflow-hidden"
      style={{ height: '100vh' }}
    >
      {/* Background video */}
      <div className="absolute inset-0 z-0 pointer-events-none">
        <video ref={videoRef} src="/safari_scrub.mp4" className="w-full h-full object-cover opacity-75" muted playsInline preload="auto" />
        <div className="absolute inset-0 bg-gradient-to-b from-[#0a0f07]/85 via-transparent to-[#0a0f07]/85" />
        <div className="absolute inset-0 bg-gradient-to-r from-[#0a0f07]/70 via-transparent to-[#0a0f07]/70" />
      </div>

      {/* Map path + vehicle */}
      <div className="absolute inset-0 z-10 pointer-events-none">
        <svg width="100%" height="100%" viewBox="0 0 1600 600" preserveAspectRatio="none">
          <path ref={pathRef} d={PATH_D} fill="none" stroke="rgba(52,211,153,0.55)" strokeWidth="2.5" strokeLinecap="round" strokeDasharray="6 10" />
        </svg>
        <div ref={vehicleRef} className="absolute w-10 h-10 -translate-x-1/2 -translate-y-1/2 opacity-0 pointer-events-none">
          <SafariCar className="w-full h-full text-emerald-400 drop-shadow-[0_4px_10px_rgba(0,0,0,0.6)]" />
        </div>
      </div>
          ...
          return (
            ...
            >
                ...
                {isTop && (
                  ...
                  >
                    <div className="flex flex-col gap-2">
                      ...
                      <button className="rv flex items-center gap-1 text-emerald-400 text-[9px] uppercase tracking-wide hover:opacity-70 transition-opacity group/btn">
              ...
              <div className="flex justify-center py-2 shrink-0">
                ...
                >
                  <span className="text-emerald-300 font-bold text-[10px] font-mono">{exp.stop}</span>
                </div>
              </div>
